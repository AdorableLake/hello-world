...
int main()
{
  ...
  int gcd,lcm;
  // 求最大公约数
  if(m<n) // 比较大小，使得较小的数存在n中，较大的数存在m中
  {
    temp=m;
    m=n;
    n=temp;
...
10. 猴子吃桃问题。一只猴子第一天摘下若干个桃子，当即吃了一半，还不过瘾，又多吃了一个；第二天早上又将剩下的桃子吃了一半，又多吃一个。以后每一天早上都吃了前一天剩下的一半加一个。到第n天早上想再吃的时候，只剩下一个桃子了。问第一天猴子摘了多少桃子？
...
11. 兔子繁衍问题。一对兔子，从出生后第三个月起每个月都生一对兔子。小兔子长到第三个月后每个月又生一对兔子。加入兔子都不死，请问第一个月出生的一对兔子，至少需要方言到第几个月时兔子中暑才可以达到n对？输入一个不超过10000的正整数，输出兔子总数达到n最少需要的月数。
